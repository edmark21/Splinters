...
def main():
  ...
  def checker():
    ...
    for i in quest_info:
      ...
      if int(rshares_limit) > rshare:
        ...
        print(style.RED + "[!] Rshare limit Detetcted")
        exit()
  ...
  try:
    ...
    transaction_id = broadcast_find_match(hive, user, "Modern Ranked", False)
    
    print(style.CYAN+"[*] Starting ")
  ...
  def send():
    ...
    if sa['battle_queue_id_1'] == transaction_id:
      if sa['winner'] == uname:
        ...
        print("[+] Time:", now.strftime('%m/%d/%Y %I:%M:%S'))
